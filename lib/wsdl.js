...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns, first, xmlnsAttr, parameterTypeObject, ancestorXmlns) {
  ...
  if (Array.isArray(obj)) {
    ...
    var params = [];
    for (name in obj) {
      allPresent = allPresent && parameterTypeObject && parameterTypeObject.description(this.definitions)[name];
      params.push(name);
    }
    if (allPresent) {
      params = [];
      for (name in parameterTypeObject.description(this.definitions)) {
        params.push(name);
      }
    }
