...
WSDL.prototype.getParameterOrder = function(obj, parameterTypeObject) {
  ...
  if (parameterTypeObject) {
    ...
    for (var name in obj) {
      allPresent = allPresent && parameterTypeObject && typeDescription[name];
      order.push(name);
    }
    if (allPresent) {
      order = [];
  ...
  if(parameterTypeObj.$lookupTypes && Array.isArray(parameterTypeObj.$lookupTypes && parameterTypeObj.$lookupTypes.length)) {
    ...
    if (parameterTypeObj.$ref) {
      var refNamespace = parameterTypeObj.$ref.substr(0, parameterTypeObj.$ref.indexOf(':')),
        refXmlns = this.definitions.xmlns[refNamespace],
        defaultXmlns = this.definitions.xmlns.xmlns,
        refLocalName = parameterTypeObj.$ref.substr(parameterTypeObj.$ref.indexOf(':') + 1);

      var elts = this.definitions.schemas[refXmlns || defaultXmlns].elements;
      var elt = elts[refLocalName];
      var typeNamespace = elt.$type.substr(0, elt.$type.indexOf(':')),
        typeXmlns = this.definitions.xmlns[typeNamespace],
        typeLocalName = elt.$type.substr(elt.$type.indexOf(':') + 1);
      found = this.findChildParameterObjectFromSchema(typeLocalName, typeXmlns);
    }
