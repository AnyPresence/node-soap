...
Element.prototype.startElement = function(stack, nsName, attrs, options) {
  ...
  var parent = stack.length === 0 ? null : stack[stack.length-1];
  if (parent && parent.xmlns && Object.keys(parent.xmlns).length > 0) {
    if (!parent.nsDeclarations) {
      parent.nsDeclarations = parent.xmlns;
    }
  }
  ...
  if (ChildClass) {
    var newChild = new ChildClass(nsName, attrs, options);
    stack.push(newChild);
    newChild.nsDeclarations = _.merge(_.cloneDeep(parent.nsDeclarations), newChild.xmlns || {});
  }
  else {
...
Element.prototype.childOrder = function(definitions) {
  return [];
};
...
Element.prototype.resolveTypeForQname = function(qname, definitions) {
  var name = splitNSName(qname);
  var localName = name.name;
  var nsPrefix = name.namespace;

  var namespace = this.nsDeclarations[nsPrefix || 'xmlns'];

  var schema = definitions.schemas[namespace];
  if (!schema) {
    return null;
  }

  return schema.types[localName] || schema.complexTypes[localName];
};

Element.prototype.resolveElementForQname = function(qname, definitions) {
  var name = splitNSName(qname);
  var localName = name.name;
  var nsPrefix = name.namespace;
...
MessageElement.prototype.postProcess = function(definitions) {
  ...
  if (part.$element) {
    ...
    for (i = 0; part = this.children[i]; i++) {
      ...
      this.parts.$order.push(part.$name);
...
ElementElement.prototype.description = function(definitions, xmlns) {
  ...
  if (type) {
    ...
    var typeName = type.name,
      ns = this.nsDeclarations[type.namespace],
      schema = definitions.schemas[ns],
      typeElement = schema && ( schema.complexTypes[typeName] || schema.types[typeName] || schema.elements[typeName] );
...
ElementElement.prototype.childOrder = function(definitions) {
  var order = [];

  // case 1 - element has a ref to another element
  if (this.$ref) {
    var referencedElt = this.resolveElementForQname(this.$ref, definitions);
    if (referencedElt) {
      order.push(referencedElt.childOrder(definitions));
    }
  }

  // case 2 - element has a type that points to an already defined complex type
  else if (this.$realType || this.$type) {
    // navigate to the type and get it's children
    var referencedType = this.resolveTypeForQname(this.$realType || this.$type, definitions);
    if (referencedType) {
      order.push(referencedType.childOrder(definitions));
    }
  }

  // case 3 - element has an anonymously defined complex type nested beneath it
  else {
    for (var i = 0; i < this.children.length; i++) {
      var child = this.children[i];
      if (child instanceof ComplexTypeElement) {
        order.push(child.childOrder(definitions));
      }
    }
  }

  return _.flatten(order);
};

RestrictionElement.prototype.childOrder = function(definitions) {
  var order = [];

  for (var i = 0; i < this.children.length; i++) {
    var child = this.children[i];
    if (child instanceof ChoiceElement || child instanceof SequenceElement) {
      order.push(child.childOrder(definitions));
    }
  }
...
ExtensionElement.prototype.childOrder = function(definitions) {
  var order = [];
  var baseType = this.resolveTypeForQname(this.$base, definitions);
  if (baseType) {
    order.push(baseType.childOrder(definitions));
  }
  for (var i = 0; i < this.children.length; i++) {
    var child = this.children[i];
...
ChoiceElement.prototype.childOrder = function (definitions) {
  var order = [];
  ...
  for (var i = 0; i < this.children.length; i++) {
    ...
    if (child instanceof ElementElement) {
      if (child.$name) {
        ...
        var refElt = this.resolveElementForQname(child.$ref, definitions);
        if (refElt && refElt.$name) {
          order.push(refElt.$name);
        }
      }
...
ComplexTypeElement.prototype.childOrder = function(definitions) {
  var order = [];
...
ComplexContentElement.prototype.childOrder = function(definitions) {
  var order = [];
...
SimpleContentElement.prototype.childOrder = function(definitions) {
  var order = [];
...
SequenceElement.prototype.childOrder = function(definitions) {
  var order = [];
...
AllElement.prototype.childOrder = function(definitions) {
  var order = [];
...
WSDL.prototype.objectToRpcXML = function(name, params, namespace, xmlns, order) {
  var self = this;
  var parts = [];
  ...
  var keys = (typeof order !== 'undefined' && order !== null) ? order : _.keys(params);
...
WSDL.prototype.getParameterOrder = function(obj, parameterTypeObject) {
  var self = this;
  var order = [];

  // if there's a parameterTypeObject describing the object, check to see if all the object's keys are in
  // the parameterTypeObject -- if so, use the natural ordering of those keys to dictate the order of the parameters.
  // in all other cases, just use the natural ordering of the keys in the payload that was passed in that will be
  // converted to the soap payload.
  if (parameterTypeObject) {
    var clone = _.clone(obj);
    var childOrder = parameterTypeObject.childOrder(this.definitions);
    for (var i = 0; i < childOrder.length; i++) {
      order.push(childOrder[i]);
      delete clone[childOrder[i]];
    }
...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns, first, xmlnsAttr, parameterTypeObject, ancestorXmlns) {
  ...
  if (typeof parentNamespace !== 'undefined' && parentNamespace !== null) {
  ...
  if (Array.isArray(obj)) {
    ...
    var params = this.getParameterOrder(obj, parameterTypeObject);
          ...
          if (schema) {
            ...
            if (childParameterTypeObject && childParameterTypeObject.$type && (childParameterTypeObject.$type.indexOf('xsd') === -1)) {
              if(childParameterTypeObject.$baseNameSpace) { //this element has a base with another namespace (the correct one)
                if (prefixNamespace && ((qualified || first) || soapHeader) && (!isNamespaceIgnored || this.ignoredNamespaces.indexOf(childParameterTypeObject.$baseNameSpace) === -1)) {
              ...
              if (childNamespace === '') { childNamespace = 'xmlns'; }
              var childXmlns = childParameterTypeObject.nsDeclarations[childNamespace];
              ...
              if (prefixNamespace && ((qualified || first) || soapHeader) && (!isNamespaceIgnored || this.ignoredNamespaces.indexOf(obj[self.options.attributesKey].xsi_type.namespace) === -1)) {
...
WSDL.prototype.findChildParameterObject = function(parameterTypeObj, childName) {
  ...
  if(parameterTypeObj.$lookupTypes && Array.isArray(parameterTypeObj.$lookupTypes && parameterTypeObj.$lookupTypes.length)) {
    ...
    if (parameterTypeObj instanceof ElementElement) {
      // Follow element refs
      if (parameterTypeObj.$ref) {
        var refNamespace = parameterTypeObj.$ref.substr(0, parameterTypeObj.$ref.indexOf(':')),
          refXmlns = parameterTypeObj.nsDeclarations[refNamespace || 'xmlns'],
          refLocalName = parameterTypeObj.$ref.substr(parameterTypeObj.$ref.indexOf(':') + 1);

        var elts = this.definitions.schemas[refXmlns].elements;
        var elt = elts[refLocalName];
        found = elt;
      } else if (parameterTypeObj.$type) {
        var referencedTypeNamespace = parameterTypeObj.$type.substr(0, parameterTypeObj.$type.indexOf(':')),
          referencedTypeXmlns = parameterTypeObj.nsDeclarations[referencedTypeNamespace || 'xmlns'],
          referencedTypeLocalName = parameterTypeObj.$type.substr(parameterTypeObj.$type.indexOf(':') + 1);

        if (this.definitions.schemas[referencedTypeXmlns]) {
          var type = this.definitions.schemas[referencedTypeXmlns].types[referencedTypeLocalName] ||
                        this.definitions.schemas[referencedTypeXmlns].complexTypes[referencedTypeLocalName];
          found = type;
