...
WSDL.prototype.getParameterOrder = function(obj, parameterTypeObject) {
  ...
  if (parameterTypeObject) {
    var allPresent = true;
    var typeDescription = parameterTypeObject.description(self.definitions);
    if (parameterTypeObject.$lookupType) {
      typeDescription = typeDescription[parameterTypeObject.$name];
    } else if (parameterTypeObject.$type && parameterTypeObject.$type.indexOf('xsd') === -1) {
      var typeName = splitNSName(parameterTypeObject.$name).name;
      if (typeDescription.$isArray) { typeName = typeName + "[]"; }
      typeDescription = typeDescription[typeName];
      // follow the double key for $types if it exists
      if (typeDescription && typeDescription[typeName]) {
        typeDescription = typeDescription[typeName];
      }
    }
    ...
    for (var name in obj) {
      var theName = name;
      if (typeDescription && typeDescription.$isArray) {
        theName += "[]";
      }
      if (name !== self.options.attributesKey && name !== self.options.valueKey && name !== self.options.xmlKey) {
        allPresent = allPresent && parameterTypeObject && typeDescription && typeDescription[theName];
      }
      order.push(name);
    }
    if (allPresent && typeof typeDescription === 'object') {
      order = [];
      for (name in typeDescription) {
        if (typeDescription.$isArray && name.indexOf('[]') !== -1) {
